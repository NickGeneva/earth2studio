...
from earth2studio.models.px.gencast_operational import (
    ATMOS_VARIABLES,
...
_TOTAL_RAM_GB = os.sysconf("SC_PHYS_PAGES") * os.sysconf("SC_PAGE_SIZE") / (1024**3)
...
def test_gencast_call(time, device, mock_GenCast_model):

    p = mock_GenCast_model.to(device)
    ...
    assert out.shape == torch.Size([len(time), 1, 84, 721, 1440])
...
def test_gencast_iter(ensemble, device, mock_GenCast_model):
    ...
    assert input.shape == torch.Size(
        [ensemble, len(time), 1, 84, 721, 1440]
    )  # 84 output vars, tp12 included
    assert len(input.shape) == 6
    for i, (out, out_coords) in enumerate(p_iter):
        assert len(out.shape) == 6
        assert out.shape == torch.Size([ensemble, len(time), 1, 84, 721, 1440])
        assert (out_coords["variable"] == p.output_coords(coords)["variable"]).all()
...
def test_gencast_exceptions(dc, device, mock_GenCast_model):
    time = np.array([np.datetime64("1993-04-05T00:00")])
    p = mock_GenCast_model.to(device)
    # Initialize Data Source
    r = Random(dc)
...
def model() -> GenCastOperational:
    package = GenCastOperational.load_default_package()
    p = GenCastOperational.load_model(package, sampler_steps=1)
    return p
...
@pytest.mark.skipif(
    _TOTAL_RAM_GB < 200,
    reason=f"GenCast Operational requires ~250 GB host RAM for JIT; system has {_TOTAL_RAM_GB:.0f} GB",
)
def test_gencast_package(model, device):
    torch.cuda.empty_cache()
    time = np.array([np.datetime64("1993-04-05T00:00")])
    # Test the cached model package gencast
    p = model.to(device)
