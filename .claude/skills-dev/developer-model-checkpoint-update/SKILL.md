...
description: >
  ...
  honoring numeric checkpoint levels, handling checkpoint device
...
6. Treat checkpoint levels `0`, `1`, and `2` as user intent hints exposed
   through the bound checkpoint proxy.
...
## Step 2 - Choose the Checkpoint Level Behavior

Implement behavior for all three checkpoint levels:

- `0`: do not store component state. The workflow may still record catalog
  progress and explicit artifacts.
- `1`: store lightweight state needed to resume a workflow item, such as an
  ensemble member, forecast instance, RNG state, counters, or replayable
  metadata. Do not store large continuation tensors here.
- `2`: store full component state needed to resume mid-rollout, including
  continuation tensors and coordinate metadata when user-facing IO is not
  restart-complete.

When a model cannot support a level, make the fallback explicit in code. For
example, level `1` may save RNG state while level `2` saves RNG plus
continuation state. Level `0` should clear or avoid updating component state.

Do not expose these levels as new model constructor parameters. Read them from
the bound state proxy:

```python
if self.restart.checkpoint_level == 2:
    ...
elif self.restart.checkpoint_level == 1:
    ...
else:
    ...
```
...
3. For level `1`, save the generator state needed to reproduce the next
   component call or forecast instance.
4. For level `2`, save the generator state that correctly continues after the
   saved mid-rollout boundary.
...
`Checkpoint(device=torch.device("cuda:0"))` to keep level 2 checkpoint tensors on
...
Only store large tensors for level `2`. For level `0` and usually level `1`,
clear large fields or leave them unset:
...
1. Construct a `Checkpoint(..., level=2)` or `level=1` as appropriate.
...
```python
checkpoint = Checkpoint("model", path=tmp_path, mode="append", level=2)
...
- Level `0`, `1`, and `2` behavior is explicit.
