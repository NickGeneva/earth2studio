...
def default_capability_inventory() -> tuple[VizCapability, ...]:
    ...
    return (
        ...
        VizCapability(
            ...
            status="partial",
            public_api_required=False,
            summary="Streaming sessions exist; summary, Matplotlib, Cartopy, and OVRTX can reconcile scene events. OVRTX now emits a browser/notebook globe session payload.",
            missing=(
                "OVRTX render-loop ownership",
                "ovstream WebRTC server lifecycle",
                "Camera synchronization",
                "Resource cleanup hooks",
                "Interactive picking/selection",
                "Concrete OpenUSD exporter/session implementation",
            ),
            next_steps=(
                "Connect the OVRTX session payload to an ovrtx plus ovstream renderer owner.",
                "Prototype a backend-owned session object under OpenUSD paths.",
