...
def _grid_for_array(array: xr.DataArray) -> tuple[str | None, GridDefinition]:
    grid_id = array.attrs.get(E2S_GRID_ID)
    if grid_id is not None:
        return _resolve_grid(grid_id)
    return None, infer_grid(array)


def _grid_metadata(array: xr.DataArray) -> dict[str, Any] | None:
    try:
        grid_id, definition = _grid_for_array(array)
    except ValueError:
        return None
    return {
        "id": grid_id,
        "registered": grid_id is not None,
        "crs": _crs_label(definition.crs),
        "spatial_dims": definition.dims,
        "shape": tuple(array.sizes[dimension] for dimension in definition.dims),
        "complete": all(
...
def _array_crs(array: xr.DataArray) -> CRS | None:
    try:
        return _grid_for_array(array)[1].crs
    except ValueError:
        return None


def _materialize_grid_indexes(array: xr.DataArray) -> xr.DataArray:
    grid_id = array.attrs.get(E2S_GRID_ID)
    if grid_id is None:
        raise ValueError("DataArray does not contain registered grid metadata")
    return array.assign_coords(_resolve_grid(grid_id)[1].index_coordinates())


def _materialize_grid_coords(array: xr.DataArray) -> xr.DataArray:
    _, definition = _grid_for_array(array)
    indexes = _coordinate_values(array.coords, definition.dims)
    return array.assign_coords(definition.geographic_coordinates(indexes))


def _subset_grid(array: xr.DataArray, selection: Mapping[str, Any]) -> xr.DataArray:
    if not selection:
        raise ValueError("At least one grid subset option is required")
    _, definition = _grid_for_array(array)
    remaining = dict(selection)
    indexers = {
        dimension: remaining.pop(dimension)
        for dimension in definition.dims
        if dimension in remaining
    }
    if any(isinstance(indexer, (int, np.integer)) for indexer in indexers.values()):
        raise ValueError("Grid subsets must preserve every spatial dimension")
    result = array.isel(indexers) if indexers else array
    if remaining:
        generated = definition.subset_indexers(result.coords, **remaining)
        unknown_dims = set(generated) - set(definition.dims)
        if unknown_dims:
            raise ValueError(
                f"Grid subset produced non-spatial indexers: {sorted(unknown_dims)}"
            )
        result = result.isel(generated)
    if any(result.sizes[dimension] == 0 for dimension in definition.dims):
        raise ValueError("Grid subset must contain at least one spatial cell")
    return result
