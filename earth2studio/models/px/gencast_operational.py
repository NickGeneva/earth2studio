...
@check_optional_dependencies()
class GenCastOperational(torch.nn.Module, AutoModelMixin, PrognosticMixin):
    """GenCast Operational diffusion-based weather prediction model.

    A stochastic weather prediction model based on conditional diffusion that predicts
    in 12-hour time steps. It supports 0.25-degree (721x1440) resolution with 13
    pressure levels. The model takes 2 input frames (t-12h and t) and predicts 12 hours
    ahead.

    The operational variant fine-tuned on HRES-fc0 data (2016-2021), which
    can be initialized from real-time operational analysis data with minimal latency,
    rather than ERA5 reanalysis which has ~5 days of latency.
    ...
    Warning
    -------
    This model requires over 64 GB of VRAM. With ``jit_compile=True``, XLA compilation
    additionally requires ~250 GB of host RAM and can take up to 20 minutes (one-time
    cost). Lowering ``sampler_steps`` (e.g. from 20 to 5) proportionally reduces
    both runtime and compilation memory at the cost of sample quality. See the
    `GenCast cloud VM setup guide
    <https://github.com/google-deepmind/graphcast/blob/main/docs/cloud_vm_setup.md>`_
    ...
    sampler_steps : int, optional
        Number of noise levels in the DPM-Solver++ 2S diffusion sampler,
        by default 20. Each noise level requires two denoiser forward passes
        (40 total at default).
    ...
    jit_compile : bool, optional
        JIT-compile the model forward pass, by default False. JIT compilation
        requires approximately 250 GB of host RAM for this operational model
        but makes subsequent calls significantly faster (one-time cost of
        several minutes).

    Badges
    ------
    region:global class:mrf product:wind product:precip product:temp product:atmos
    product:ocean year:2024 gpu:80gb
    """
    ...
    def load_default_package(cls) -> Package:
        """Load default pre-trained GenCast Operational model package from Google Cloud.
    ...
    def load_model(
        ...
        # Load checkpoint
        params_path = package.resolve("params/GenCast 0p25deg Operational <2022.npz")
        ...
        sample_input = xr.load_dataset(
            package.resolve(
                "dataset/source-hres_date-2022-03-29_res-0.25_levels-13_steps-01.nc"
            )
        )
    ...
    def _load_run_forward_from_checkpoint(
        self, jit_compile: bool = False, sampler_steps: int = 20
    ) -> Callable:
        ...
        sampler_config = dataclasses.replace(
            sampler_config, num_noise_levels=sampler_steps
        )
